...
export const StoreCheckoutClient = ({ storeSlug }: { storeSlug: string }) => {
  ...
  // Filter cart to only this store's items
  const storeItems = cart.filter((i) => i.storeSlug === storeSlug);
  const store = MOCK_STORES.find((s) => s.slug === storeSlug);
  const storeName = store?.name ?? storeItems[0]?.storeName ?? storeSlug;
  ...
  // If no items for this store, redirect back
  if (storeItems.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 px-6 text-center pb-24">
        <p className="text-slate-500 font-semibold">Não há itens desta loja no carrinho.</p>
        <Link href="/carrinho" className="px-6 py-3 bg-slate-900 text-white rounded-full font-black text-sm">
          Voltar ao Carrinho
        </Link>
      </div>
    );
  ...
  const handleSend = () => {
    ...
    const itemsFormatted = storeItems
      .map((item) => {
        const vars: string[] = [];
    ...
    // Save history, persist user info, remove these items from cart
    addOrderToHistory(storeItems, storeName);
    updateUserInfo({ name: name.trim(), phone: phone.trim(), location: address.trim() });

    // Remove only this store's items from cart
    const globalIndexes = cart
      .map((item, i) => (item.storeSlug === storeSlug ? i : -1))
      .filter((i) => i !== -1)
      .reverse(); // reverse so removing doesn't shift indexes
    globalIndexes.forEach((i) => removeFromCart(i));
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-4 pb-36 space-y-5">
      ...
      <Link
        href={`/loja/${storeSlug}`}
        className="flex items-center gap-3 p-4 bg-white rounded-3xl border border-slate-100 shadow-sm hover:bg-slate-50 transition-colors"
      >
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          ...
          <div className="flex items-center gap-3 px-4 py-3.5">
            ...
            <div className="flex-1">
              <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-1">
                Endereço de entrega <span className="font-medium normal-case tracking-normal">(opcional)</span>
              </label>
              <input
                type="text"
                value={address}
