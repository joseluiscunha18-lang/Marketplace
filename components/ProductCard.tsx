...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <Link
      ...
      className="group bg-white rounded-[18px] overflow-hidden flex flex-col transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-1 active:scale-[0.98] border border-slate-200/60"
    >
      <div className="relative w-full aspect-square bg-slate-50 overflow-hidden border-b border-slate-200/60">
        <Image
          src={product.image}
          alt={product.name}
          fill
        ...
        <button
          ...
          className={`absolute top-3 right-3 z-20 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 active:scale-90 shadow-sm backdrop-blur-md ${
            isFav
      ...
      <div className="px-3.5 pt-3 pb-3.5 flex flex-col flex-grow">
        ...
        <div className="flex items-baseline gap-1.5 mt-0.5">
          <span className="font-extrabold text-[15px] text-slate-900 tracking-tight flex items-baseline">
            {product.price.toLocaleString('pt-MZ')}
            <span className="text-[13px] font-bold text-slate-800 ml-1">MT</span>
          </span>
